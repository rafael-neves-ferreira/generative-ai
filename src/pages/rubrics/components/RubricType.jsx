    ...
    return (
        < div className='flex justify-between' key={props.rubric.id} >
            ...
            <div className='flex space-x-4 justify-end'>
                <p>NB caracteres:</p>
                <input type="number" value={props.rubric.defaultValue} max={1500} onChange={(e) => { props.changeRubricDefaultValue(e.target.value, props.rubric.id) }} className='border w-2/12 text-center h-6' />
            </div>
        </div >
    )
}
...
const RubricType2 = (props) => {
    return (
        < div className='flex justify-between' key={props.rubric.id} >
            ...
            <div className='flex space-x-4 justify-end'>
                <p>NB caracteres:</p>
                <input type="number" value={props.rubric.defaultValue} max={1500} maxLength={100} onChange={(e) => { props.changeRubricDefaultValue(e.target.value, props.rubric.id) }} className='border w-2/12 text-center h-6' />
            </div>
        </div >
    )
}
